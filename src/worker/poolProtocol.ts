 ...
 * (the main-thread <-> coordinator protocol, unchanged) because this one
 * only ever travels worker-to-worker.
 *
 * Each pool worker (including the coordinator, for its own local share)
 * evaluates slices via `evaluatePriceSlice` (src/worker/pricing.ts) — a
 * pure, synchronous, structured-cloneable-in/out function, so a slice job
 * is just its plain arguments and a slice result is just a McRunResult.
 */
