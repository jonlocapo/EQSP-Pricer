 ...
 *   of "80" to type "70" transiently reads 0), so it wants a LONGER wait, or
 *   the engine burns a full solve on a number the user never intended.
 *
 * This lives outside the React store deliberately: it's transient input
 * metadata, not application state, and putting it in the store would trigger
 * a re-render of every subscriber on each keystroke for no benefit.
 */

export type EditSource = 'type' | 'step';

/** How long a source marker stays trustworthy. Any edit that reaches the
 * live-reprice hook later than this didn't come from a field we instrumented
 * (a segmented control, a preset button, a market-data fetch), so it falls
 * back to the conservative longer wait. */
 ...
 * is stale (see FRESHNESS_MS). Non-destructive — several subscribers may read
 * it for the same edit. */
